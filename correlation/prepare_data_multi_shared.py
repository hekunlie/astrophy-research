...
if cmd == "grid":
    ...
    cf_cata_data_path = data_path + "cf_cata_%s_multi.hdf5" % result_source
    logger.info("Start....")
    for area_id in range(area_num):
        ...
        if rank == 0:
            ...
            data_row = len(mg1)
            buffer = numpy.zeros((data_row, data_col))
            ...
            # the size of shared memory buffer in unit of byte
            nbytes = data_row*data_col*elem_size
            shared_size = data_row*data_col
            # the list can store INT and DOUBLE simultaneously !!! if cant't,  be careful
            bcast_info = [data_row, data_col, grid_rows, grid_cols, ra_min, ra_max, dec_min, dec_max]
        else:
            nbytes = 0
            bcast_info = None
        ...
        data_row, data_col, grid_rows, grid_cols, = bcast_info[0:4]
        ra_min, ra_max, dec_min, dec_max = bcast_info[4:8]
        grid_num = grid_rows*grid_cols

        logger.info("Create the shared buffer, %d x %d"%(data_row, data_col))

        win = MPI.Win.Allocate_shared(nbytes, elem_size, comm=comm)
        shared_men, itemsize = win.Shared_query(0)
        buffer = numpy.ndarray(buffer=shared_men,dtype=numpy.double, shape=(data_row, data_col))

        comm.Barrier()
        # rank 0 initialize the shared memory buffer for all the threads
        if rank == 0:
            buffer[:, 0] = ra
            buffer[:, 1] = dec
            ...
            buffer[:, 6] = mv
        comm.Barrier()
        # all threads can see the buffer now
        # check
        err_tag = 1
        if numpy.abs(numpy.sum(buffer[:100000,0])) <= 0.00001:
            print("Rank: %d. Buffer is empty"%rank)
            err_tag = 0
        logger.info("Check the buffer. ERR: %d"%err_tag)
        err_tags = comm.allgather(err_tag)
        if sum(err_tags) != cpus:
            exit()

        # then, distribute the grids to threads
        tasks_list = [i for i in range(grid_rows * grid_cols)]
