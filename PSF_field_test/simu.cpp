...
{	
	...
	create_psf_e(psf_img[2], psf_scale, size, img_cent, psf_ellip*0.7, 0.174, psf_type);
	...
	for (shear_id = 0; shear_id < shear_pairs; shear_id++)
	{
		...
		for (i = chip_st; i < chip_ed; i++)
		{
			...
			{	
				...
				shear_est(stamp_pow_img[0], psf_img[3], &all_paras);				
		...
		if (0 == rank)
		{
			sprintf(result_path, "%s/data/data_noise_free_psf_r_%d.hdf5", parent_path, shear_id);
			write_data(result_path, total_data, mg_name, mg_data, total_data_row, shear_data_cols);
		}
		MPI_Barrier(MPI_COMM_WORLD);
