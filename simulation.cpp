...
int main(int argc, char*argv[])
{
		...
		int chip_num = 1000, stamp_num = 10000, shear_pairs = 14;
		/* remember to change the data_cols when you change the number of estimators recorded */
		int i, j, seed, data_rows, data_cols=28;
		...
		sprintf(buffer, "/home/hklee/work/selection_bias/parameters/ellip_%d.dat", myid);
		fin.open(buffer);
		i = 0;
		while (!fin.eof())
		{
		...
		fin.close();
		sprintf(buffer, "%d: opened ellip_%d.dat and got %d ellips ", myid, myid, i);
		cout << buffer << endl;
		 //PSF
		...
		{	
			...
			for (j = 0; j < stamp_num; j++)
			{
				...
				data[i*stamp_num + j][16] = all_paras.gal_flux;
				data[i*stamp_num + j][17] = all_paras.gal_peak;
				...
				data[i*stamp_num + j][22] = all_paras.gal_snr;
				data[i*stamp_num + j][23] = (double)(myid);
				data[i*stamp_num + j][24] = (double)(i);
				data[i*stamp_num + j][25] = (double)(j);
			}
