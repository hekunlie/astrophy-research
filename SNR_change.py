...
for k in range(len(flux)):
    ...
    gal = gal.shear(e=e, beta=0.*galsim.degrees)
    ...
    for i in range(num):
        ...
        gal_s = gal_f.shear(g=0.06, beta=shear_beta[i]*galsim.radians)
        ...
        hdu.writeto(path, overwrite=True)
title = "xy_%d\\pr_%.2f\\gr_%.2f\\e%.2f\\btr_%.2f\\seed_%d"%(size, psf_r,ra, e, btr, seed)
