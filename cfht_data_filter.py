...
try:
    ...
    for c in contents:
        name = c.split("\n")[0] + "_shear.dat.npz"
        files.append(name)
    print("Field num: %d" % len(files))

except:
    print("Filtered file dose not exist! %s"%filter_path)
    exit()
...
for name in files:
    chi_1s = []
    chi_2s = []
    new_path = field_path + name.split("_")[0]+ "/" + name
...
for w_name in all_fields:
    if w_name not in filtered:
        excluded.append(w_name)
