...
for fnm in files:
    if ".hdf5" in fnm:
        for j in range(len(sub_samples)):
            ...
            cmd = "mpirun -np 50 ./GGL_cal /home/hklee/work/DECALS/gg_lensing %d 0.2"%sub_samples[j]
            a = Popen(cmd, shell=True)
            a.wait()
            time.sleep(2)

            src = total_path + "/result/result.hdf5"
            dst = total_path + "/result_jessebin/%s_%d_sub_sample.hdf5"%(fnm.split("_DECALS")[0], sub_samples[j])
            shutil.move(src, dst)
            #
            # print("\nmove to %s\n"%dst)
            #src = total_path + "/result/result.hdf5"
