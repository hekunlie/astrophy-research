...
for j in range(1):
    for fnm in [files[1]]:
        if ".hdf5" in fnm:
            ...
            cmd = "mpirun -np 1 python prepare_cata_sym.py prepare_foreground %s"%fnm.split("\n")[0]
            a = Popen(cmd, shell=True)
            a.wait()
            time.sleep(2)
            
            cmd = "mpirun -np 1 python prepare_cata_sym.py prepare_pdf"
            a = Popen(cmd, shell=True)
            a.wait()
            time.sleep(2)
            ...
            dst = total_path + "/result_jesse/guess_test/sym/%s_%d.hdf5"%(fnm.split("_DECALS")[0],j)
            shutil.move(src, dst)
            #
            # print("\nmove to %s\n"%dst)
            #src = total_path + "/result/result.hdf5"
