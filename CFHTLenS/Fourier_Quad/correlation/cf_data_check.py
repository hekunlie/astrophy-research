...
num_in_block = h5f["/grid/w_%d/num_in_block"%area_id].value
block_start = h5f["/grid/w_%d/block_start"%area_id].value
block_end = h5f["/grid/w_%d/block_end"%area_id].value

mg1 = h5f["/grid/w_%d/data/G1"%area_id].value
mg2 = h5f["/grid/w_%d/data/G2"%area_id].value
mn = h5f["/grid/w_%d/data/N"%area_id].value
mu = h5f["/grid/w_%d/data/U"%area_id].value
mv = h5f["/grid/w_%d/data/V"%area_id].value
ra = h5f["/grid/w_%d/data/RA"%area_id].value
dec = h5f["/grid/w_%d/data/DEC"%area_id].value

data_set = [mg1, mg2, mn, mu, mv, ra, dec]

set_nms = ["G1", "G2", "N", "U", "V", "RA", "DEC"]
check_num = numpy.zeros((grid_num, 2*len(set_nms)))

for row in range(grid_ny):
    for col in range(grid_nx):
        grid_id = row*grid_nx + col
        group_nm = "/grid/w_%d/row_%d/col_%d"%(area_id, row, col)
...
print(rank, grid_shape, numpy.sum(check_num))
