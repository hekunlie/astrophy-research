...
int main(int argc, char*argv[])
{
	...
	std::ifstream fin;
	std::string s, detect_info, str_stampsize = "stamp_size", str_total_num = "total_num", str_noise = "noise_sig", str_shear_num = "shear_num", str_nx = "stamp_col";
	...
	for (shear_id = shear_pairs; shear_id < shear_pairs+2; shear_id++)
	{
		ts = clock();
		...
		sprintf(para_path, "%sparameters/para_%d.hdf5", data_path, 0);
		sprintf(set_1, "/flux");
		sprintf(set_2, "/mag");
		...
		for (i = chip_id_s; i < chip_id_e; i++)
		{
			...
			seed = myid * i + shear_id + 58490 + i + temp_s;
			temp_s++;
			...
			for (j = 0; j < stamp_num; j++)
			{
				...
				if (shear_id == shear_pairs)
				{
					create_points(point, num_p, max_radius);
					flux_i = flux[i*stamp_num + j] / num_p;

					initialize_arr(gal, size*size, 0);
					convolve(gal, point, flux_i, size, num_p, 0, psf_scale, 0, 0, psf_type, 1, &all_paras);
				}
