...
parent_path = "E:/works/Group_meeting/2019-11-25-shear_bias_checking/result/13"
mc1 = numpy.zeros((len(bin_num), 4))
mc2 = numpy.zeros((len(bin_num), 4))
for ig, num in enumerate(bin_num):
    h5f = h5py.File(parent_path + "/result_noisy_img_bin_num_%d.hdf5"%num,"r")
    ...
    img1_path = parent_path + "/result_noisy_img_bin_num_%d.png"%num
...
for i in range(2):
    img.del_tick(0,i,[1])
img.save_img(parent_path+"/result_noisy_img_bin_num_compare.png")
img.show_img()