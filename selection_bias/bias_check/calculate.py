...
result_name = total_path + "/result_%s.hdf5"%noise_type
img_check_name = total_path + "/data_%d_"+"%s.png"%noise_type
result_img_name = total_path + "/result_%s.png"%noise_type
...
for ig in my_task:
    ...
    left, right = -0.08, 0.08
    g1_mean, g1_sig_mean = fq.find_shear_mean(mg1, mn)
    g2_mean, g2_sig_mean = fq.find_shear_mean(mg2, mn)

    g1_sym, g1_sig_sym = fq.find_shear(mg1, mnu1, 8, left=left, right=right, fit_num=20, chi_gap=40,
                                       fig_ax=img_check.axs[0][0])[:2]
    g2_sym, g2_sig_sym = fq.find_shear(mg2, mnu2, 8, left=left, right=right, fit_num=20, chi_gap=40,
...
if rank == 0:
    ...
    img = Image_Plot(fig_x=6,fig_y=4,xpad=0.25,ypad=0.5)
    img.subplots(1,2)
    img.axs[0][0].errorbar(mean_result_array[0],mean_result_array[1],mean_result_array[2],
                           marker="s",fmt=" ",label="g1",capsize=img.cap_size)
    img.axs[0][0].errorbar(mean_result_array[3],mean_result_array[4],mean_result_array[5],
                           marker="s",fmt=" ",label="g2",capsize=img.cap_size)
    mc_str = "Mean\n$m_1 = %.5f (%.5f)$\n$c_1 = %.5f (%.5f)$\n$m_2 = %.5f (%.5f)$\n$c_2 = %.5f (%.5f)$"\
             %(mean_mc_array[0,0],mean_mc_array[0,1],mean_mc_array[0,2],mean_mc_array[0,3],
               mean_mc_array[1,0],mean_mc_array[1,1],mean_mc_array[1,2],mean_mc_array[1,3])
    img.axs_text(0,0,0.8,0.05, mc_str,text_fontsize=img.legend_size)

    img.axs[0][1].errorbar(sym_result_array[0],sym_result_array[1],sym_result_array[2],
                           marker="s",fmt=" ",label="g1",capsize=img.cap_size)
    img.axs[0][1].errorbar(sym_result_array[3],sym_result_array[4],sym_result_array[5],
                           marker="s",fmt=" ",label="g2",capsize=img.cap_size)
    mc_str = "PSF_SYM\n$m_1 = %.5f (%.5f)$\n$c_1 = %.5f (%.5f)$\n$m_2 = %.5f (%.5f)$\n$c_2 = %.5f (%.5f)$"\
             %(sym_mc_array[0,0],sym_mc_array[0,1],sym_mc_array[0,2],sym_mc_array[0,3],
               sym_mc_array[1,0],sym_mc_array[1,1],sym_mc_array[1,2],sym_mc_array[1,3])
    ...
    for i in range(2):
        ...
        img.axs[0][i].plot([-0.1,0.1],[-0.1,0.1],ls="dashed",c="grey",alpha=0.5)
        img.axs[0][i].set_ylim(-0.036,0.036)
        img.axs[0][i].set_xlim(-0.036,0.036)
    img.save_img(result_img_name)
