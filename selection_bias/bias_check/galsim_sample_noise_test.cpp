...
{	
	...
	char buffer[200], log_inform[250], set_name[50], psf_path[200];
	char galsim_img_path[200];

	//sprintf(parent_path, "/mnt/perc/hklee/bias_check");
	//sprintf(parent_path, "/mnt/ddnfs/data_users/hkli/bias_check");
	sprintf(parent_path, "/lustre/home/acct-phyzj/phyzj-sirius/hklee/work/selection_bias/bias_check/galsim_sample");
	...
	double *galsim_img = new double[stamp_nx*stamp_nx*size*size]{};
	...
	seed_pts = sss1*rank + 1 + 30000;
	seed_n1 = sss1*rank + 1 + 601100*(rotation+1);
	seed_n2 = sss1*rank + 1 + 305100*(rotation+1);
	...
	// create PSF
	sprintf(psf_path,"%s/psf.fits",parent_path);
	read_fits(psf_path, psf);
	...
	for (shear_id = 0; shear_id < shear_pairs; shear_id++)
	{
		...
		for (i = chip_st; i < chip_ed; i++)
		{
			...
			// read galsim noise-free image
			sprintf(galsim_img_path, "%s/%d/gal_chip_%04d.fits",parent_path, shear_id, i);
			read_fits(galsim_img_path, galsim_img);
			...
			{	
				...
				segment(galsim_img, gal, j, size, stamp_nx, stamp_nx);
