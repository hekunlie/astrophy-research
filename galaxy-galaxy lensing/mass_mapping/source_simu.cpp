...
{	
    ...
    sprintf(para_path,"%s/source_data.hdf5", parent_path);
    sprintf(set_name,"/g1");
    ...
    for (i = chip_st; i < chip_ed; i++)
    {
        ...
        if(rank == 0 and i < 3)
        {
            sprintf(chip_path, "!%s/gal_chip_%05d_noise_free.fits", parent_path, i);
    ...
    if (0 == rank)
    {
        sprintf(result_path, "%s/data_noise_free.hdf5", parent_path);
    ...
    if (0 == rank)
    {
        sprintf(result_path, "%s/data_noisy_cpp.hdf5", parent_path);
        sprintf(set_name,"/data");
        write_h5(result_path, set_name, total_data, total_data_row, shear_data_cols,true);
    }
    MPI_Barrier(MPI_COMM_WORLD);
