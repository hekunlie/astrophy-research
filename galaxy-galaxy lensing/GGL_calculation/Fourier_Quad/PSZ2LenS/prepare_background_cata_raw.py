...
if cmd == "collect":
    ...
    for area_id in range(1, area_num+1):
        ...
        for field_name in field_pool:
            ...
            cache_nm = fourier_cata_path + "%s/result_ext/%s_shear.hdf5"%(field_name, field_name)
            # for the new version catalog
            # f_data_path = fourier_cata_path + "%s/%s_shear_raw.cat"%(field_name, field_name)
            # cache_nm = fourier_cata_path + "%s/%s_shear_raw.hdf5"%(field_name, field_name)
            ...
            if os.path.exists(f_data_path) and os.path.exists(c_data_path):
                try:
                    ...
                    f_sp = f_data.shape
                    # the redshift ...., if non-exist in the cfht catalog, labeled by -1
                    ...
                    h5f_cache = h5py.File(cache_nm,"w")
                    h5f_cache["/data"] = new_data
                    h5f_cache.close()
...
if cmd == "select":
    ...
    if rank == 2:
        ...
        cut_idx = idx_select & fg1_idx & fg2_idx & idxz_1 & idxz_2

        fg1 = cata_data[:, field_g1_lb][cut_idx]
        fg2 = cata_data[:, field_g2_lb][cut_idx]
