...
# plot the GGL signal in background redshift bin [z1, z2]
...
if area_num > 1:
    ...
    result_path = parent_result_path + "result/%s_result_w_%d.hdf5"%(fore_source, area_id)
    result_path_npz = parent_result_path + "result/%s_result_w_%d.npz"%(fore_source, area_id)
    dens_pic_path = parent_result_path + "result/%s_w_%d"%(fore_source, area_id)
    dens_r_pic_path = parent_result_path + "result/%s_w_%d_sigmaxr"%(fore_source, area_id)
...
if cmd == "calculate":
    result = numpy.zeros((6, radius_num))
    for ir in range(radius_num):

        stack_count = 0
        for ia in range(5, argc):
        ...
        if stack_count > 0 and idx.sum() > 0:
            ...
            result[los_dist_lb, ir] = cosmos.comoving_distance(redshift.mean()).value*h
