...
for m in range(2):
    ...
    for i in range(4):
        ...
        axs[ax_tag].scatter(range(1, len(xi_t) + 1), xi_t, marker="s", label="Total", facecolor="none", linewidths=2,
                            color="C0", s=80)
        axs[ax_tag].plot(range(1, len(xi_t) + 1), xi_t, color="C0")
        if i == 0 or i == 4:
            idx = detect_f
            ...
            axs[ax_tag].scatter(range(1, len(xi) + 1), xi, marker="s", label="Detected", facecolor="none", linewidths=2,
                                color="C1", s=80)
            axs[ax_tag].plot(range(1, len(xi) + 1), xi, color="C1")
        else:
            idx = detect_s
        ...
        for j in range(2):
            ...
            #             axs[ax_tag].errorbar(range(1,len(xi)+1), xi,  marker="s", label=lb,facecolor="none",linewidths=2)
            axs[ax_tag].scatter(range(1, len(xi) + 1), xi, marker="s", label=lb, facecolor="none", linewidths=2,
                                color="C%d" % (j + 2), s=80)
            axs[ax_tag].plot(range(1, len(xi) + 1), xi, color="C%d" % (j + 2))
        ...
        if xs[0] < plt_xs[0]:
            plt_xs[0] = xs[0]
        axs[ax_tag].legend(loc=locs[m], fontsize=legend_size, frameon=False, bbox_to_anchor=legend_loc[m])
    ...
    for i in range(4):
        ...
        axs[ax_tag].set_ylim(plt_ys[0] - 10, plt_ys[1] + 10)
        if i > 0:
            ...
            if "e1" == cmd:
                e_label = "$\chi^2_{e_1}$ ($g_1$=%.2f)" % g1
            else:
                e_label = "$\chi^2_{e_2}$ ($g_2$=%.2f)" % g2
        ...
        axs[ax_tag].tick_params(direction='in', labelsize=xy_tick_size, top=True, right=True, pad=7)
...
plt.savefig("chisq_pts.pdf", bbox_inches='tight')
plt.show()
